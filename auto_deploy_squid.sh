...
configure_tunnel() {
    ...
    rm -f /etc/rc.local
    rm -f /opt/scripts/init_squid.sh
    touch /etc/rc.local
    touch /opt/scripts/init_squid.sh


    # Load mod
    modprobe ip_gre
    echo "modprobe ip_gre" >> /opt/scripts/init_squid.sh

    # Flush tunnel configuration
    ip tunnel del line1 &> /dev/null
    ip tunnel del line2 &> /dev/null
    echo "ip tunnel del line1" >> /opt/scripts/init_squid.sh
    echo "ip tunnel del line2" >> /opt/scripts/init_squid.sh
    ...
    echo "ip tunnel add line1 mode gre remote $remote_ip1 local $ipv4_address dev eth0" >> /opt/scripts/init_squid.sh
    echo "ip tunnel add line2 mode gre remote $remote_ip2 local $ipv4_address dev eth0" >> /opt/scripts/init_squid.sh

    ip link set line1 up
    ip link set line2 up

    echo "ip link set line1 up" >> /opt/scripts/init_squid.sh
    echo "ip link set line2 up" >> /opt/scripts/init_squid.sh
    ...
    echo "echo \"1\" > /proc/sys/net/ipv4/ip_forward" >> /opt/scripts/init_squid.sh
    echo "echo \"0\" > /proc/sys/net/ipv4/conf/default/rp_filter" >> /opt/scripts/init_squid.sh
    echo "echo \"0\" > /proc/sys/net/ipv4/conf/all/rp_filter" >> /opt/scripts/init_squid.sh
    echo "echo \"0\" > /proc/sys/net/ipv4/conf/eth0/rp_filter" >> /opt/scripts/init_squid.sh
    echo "echo \"0\" > /proc/sys/net/ipv4/conf/lo/rp_filter" >> /opt/scripts/init_squid.sh
    echo "echo \"0\" > /proc/sys/net/ipv4/conf/line1/rp_filter" >> /opt/scripts/init_squid.sh
    echo "echo \"0\" > /proc/sys/net/ipv4/conf/line2/rp_filter" >> /opt/scripts/init_squid.sh


    # Flush iptables nat table
    iptables -F -t nat
    echo "iptables -F -t nat" >> /opt/scripts/init_squid.sh
    ...
    echo "iptables -t nat -A PREROUTING -i line1 -p tcp -m tcp --dport 80 -j DNAT --to-destination $ipv4_address:3128" >> /opt/scripts/init_squid.sh
    echo "iptables -t nat -A PREROUTING -i line2 -p tcp -m tcp --dport 80 -j DNAT --to-destination $ipv4_address:3128" >> /opt/scripts/init_squid.sh
