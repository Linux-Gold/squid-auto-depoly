...
count=$((opt_qingdao + opt_jinan + opt_lnk + opt_bgl))
if [ $count -gt 1 ]; then
...
if [ $opt_qingdao -eq 1 ]; then
  ...
  sed -i "s/^wccp2_router .*/wccp2_router $wccp2_router $wccp2_router2/" /etc/squid/squid.conf
...
if [ $opt_bgl -eq 1 ]; then
  ...
  ipv4_address=$(ip -4 addr show eth0 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')
  ufdb_control="1.1.1.1/8"
  acl_localnet_src="2.2.2.2/8"
  wccp2_router1="3.3.3.3"
  wccp2_router2="4.4.4.4"
  dns_nameservers1="5.5.5.5"
  dns_nameservers2="6.6.6.6"
  remote_ip1="7.7.7.7"
  remote_ip2="8.8.8.8"
